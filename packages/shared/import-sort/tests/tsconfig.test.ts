...
describe('internalPatternsFromTsconfig', () => {
  it('turns wildcard aliases into prefix patterns and exact aliases into anchored ones', () => {
  ...
  it('tolerates comments and trailing commas, as tsc does', () => {
    ...
    expect(internalPatternsFromTsconfig({ rootDir: directory }, join(directory, 'a.ts'))).toEqual([String.raw`^#lib/`]);
  });
